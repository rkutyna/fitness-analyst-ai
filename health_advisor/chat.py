...
def answer_question(ctx: VaultContext, question: str, *, as_of: str | None = None,
    ...
    ``evidence_status``, when supplied, is a Python-decided reason ``cite``
    was withheld for THIS turn even though a corpus is configured (the
    domain gate, #394 step 2) -- e.g. ``"out_of_corpus_domain"`` or
    ``"corpus_domain_unverified"``. It is published as a closed
    ``evidence:status`` fact (``fact_template.build_evidence_status_fact``)
    so the model states the absence rather than asserting or inventing a
    source ("absence is not a fact"). ``None`` means either no corpus is
    configured at all, or this turn's question passed the gate and ``cite``
    is present -- both have nothing to publish.
    """
    from . import llm
...
def _answer_fact_template(ctx: VaultContext, question: str, prompt: str,
    ...
    # published" as "the model has a figure to interpolate" -- that would
    # force a GaAs-style question with no vault data into the zero-figure
